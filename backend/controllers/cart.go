...
func AddItemToCart(c *gin.Context) {
	...
	userID := userIDInterface.(int)
	cartID, err := getOrCreateCartIDByUserID(userID)
	if err != nil {c.JSON(http.StatusInternalServerError, gin.H{"error": "Could not process user cart"})
		return
		}
	...
	if err := c.ShouldBindJSON(&req); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request data"})
		return
	}
	...
	if err != nil {c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to add item to cart"})
		return
	}
	GetCart(c)
}
